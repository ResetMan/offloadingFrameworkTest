...
//	}
	...
	public static byte[] Math(byte[] oriBmpbs, byte[] tagBmpbs) {

		Bitmap oriBmp = Tools.Bytes2Bitmap(oriBmpbs);
		Bitmap tagBmp = Tools.Bytes2Bitmap(tagBmpbs);
		...
		for (Y = 1; Y <= H - 2; Y++) {
			for (X = 1; X <= W - 2; X++) {
				...
				if (!Loop) {
					...
					if (PlateNumberGroup.AlreadyChecked) return Tools.Bitmap2Bytes(bitmap);
				}
			}
		}
