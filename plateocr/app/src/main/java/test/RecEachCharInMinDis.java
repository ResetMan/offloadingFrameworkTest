...
//	}
	...
	public static byte[] CreateBmp(int ALeft, int ARight, int ATop, int ABottom, byte[] ABmpbs) {

		Bitmap ABmp = Tools.Bytes2Bitmap(ABmpbs);
		int W = ARight - ALeft + 1, H = ABottom - ATop + 1;
		Bitmap bitmap = Bitmap.createBitmap(W, H, Bitmap.Config.ARGB_8888);
		int CQ[] = new int[W * H];
		int posCQ = 0;
		...
		return Tools.Bitmap2Bytes(bitmap);
	}

	public static byte[] GetRegion(byte[] Bmpbs) {
		...
		return CreateBmp(Left, Right, Top, Bottom, Tools.Bitmap2Bytes(Bmp));

	}

	public static byte[] Zoom(byte[] Bmpbs) {

		Bitmap Bmp = Tools.Bytes2Bitmap(Bmpbs);
		return Tools.Bitmap2Bytes(Bitmap.createScaledBitmap(Bmp, 32, 32, false));
	}

	public static byte[] ClearSmall(byte[] Bmpbs) {
	...
	public static String XDisRec(int AZFNum, String AFName, String[] AE, byte[] ABmpbs) {
	...
	public static String MinDisRec(int AZFNum, String AFName, String[] AE, byte[] ABmpbs) {
	...
	public static String GetHZ(byte[] Bmpbs) {

		String RLT1 = "", RLT2 = "";
		String FName = "h";
		Bmpbs = GetRegion(Bmpbs);
		Bmpbs = Zoom(Bmpbs);
		RLT1 = MinDisRec(32, FName, PlateNumberGroup.E1, Bmpbs);
		RLT2 = XDisRec(32, FName, PlateNumberGroup.E1, Bmpbs);

		return RLT1 + "," + RLT2;
	}

	public static String GetZM(byte[] Bmpbs) {

		String FName = "z";
		Bmpbs = ClearSmall(Bmpbs);
		Bmpbs = GetRegion(Bmpbs);
		Bmpbs = Zoom(Bmpbs);
		return MinDisRec(25, FName, PlateNumberGroup.E3, Bmpbs);
	}

	public static String GetSZ(byte[] Bmpbs) {

		String FName = "s";
		Bmpbs = ClearSmall(Bmpbs);
		Bmpbs = GetRegion(Bmpbs);
		Bmpbs = Zoom(Bmpbs);
		return MinDisRec(33, FName, PlateNumberGroup.E2, Bmpbs);
	}

	public static String Math(byte[][] bitmapsbs) {

		String CPH = "", HZ1 = "", HZ2 = "", ZM = "", SZ1 = "", SZ2 = "", SZ3 = "", SZ4 = "", SZ5 = "";

		String HZ = GetHZ(bitmapsbs[0]);
		String[] HZs = HZ.split(",");
		HZ1 = HZs[0];
		HZ2 = HZs[1];

		ZM = GetZM(bitmapsbs[1]);

		SZ1 = GetSZ(bitmapsbs[2]);
		SZ2 = GetSZ(bitmapsbs[3]);
		SZ3 = GetSZ(bitmapsbs[4]);
		SZ4 = GetSZ(bitmapsbs[5]);
		SZ5 = GetSZ(bitmapsbs[6]);
